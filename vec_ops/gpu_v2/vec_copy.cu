...
__global__ void copy(index_t n, value_t *x, value_t *y)
{
    index_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n / 2)
        reinterpret_cast<double2 *>(y)[i] = reinterpret_cast<double2 *>(x)[i];
...
struct vec_copy_cuda : public vec_bench
{
    void benchmark()
    {
        memory_transfer_per_loop = 2.0 * sizeof(value_t) * double(vec_size) /
        ...
        {
            copy<<<grid, block>>>(vec_size, d_x, d_y);
        ...
        test_result(y, vec_size, value_t(vec_size));
        std::cout << "  y[0] " << y[0] << '\n';
    ...
    vec_copy_cuda(int narg, char **arg) : vec_bench(narg, arg) {}
};

int main(int narg, char **arg)
{
    check_cuda_device();
    vec_copy_cuda test(narg, arg);
    test.benchmark();
}