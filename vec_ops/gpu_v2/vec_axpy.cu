...
__global__ void axpy(index_t n, value_t scale, value_t *x, value_t *y)
{
...
struct vec_axpy_cuda : public vec_bench
{
    void benchmark()
    {
        ...
        {
            axpy<<<grid, block>>>(vec_size, a, d_x, d_y);
    ...
    vec_axpy_cuda(int narg, char **arg) : vec_bench(narg, arg) {}
};

int main(int narg, char **arg)
{
    check_cuda_device();
    vec_axpy_cuda test(narg, arg);
    test.benchmark();
}