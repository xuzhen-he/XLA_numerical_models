...
def predictor(u, v, p, \
    ...
    pstar_x0ya =jnp.expand_dims(p[1, 1:-1] \
                -c2_dtdx * (u[2, 1:-1] - u[1, 1:-1]) \
                -c2_dtdy * (v[1, 2:] - v[1, 1:-1]),axis=1)
    # print(pstar_x0ya.shape)
    pstar_xendya = jnp.expand_dims(p[-2, 1:-1] \
                -c2_dtdx * (u[-1, 1:-1] - u[-2, 1:-1]) \
                -c2_dtdy * (v[-2, 2:] - v[-2, 1:-1]),axis=1)
    # print(pstar_xendya.shape)
    pstar_xay0 = jnp.expand_dims(jnp.r_[0.0,
                p[1:-1, 1] \
                -c2_dtdx * (u[2:, 1] - u[1:-1, 1]) \
                -c2_dtdy * (v[1:-1, 2] - v[1:-1, 1]),
                0.0],axis=0)
    # print(pstar_xay0.shape)
    pstar_xayend = jnp.expand_dims(jnp.r_[0.0,
                p[1:-1, -2] \
                -c2_dtdx * (u[2:, -2] - u[1:-1, -2]) \
                -c2_dtdy * (v[1:-1, -1] - v[1:-1, -2]),
                0.0],axis=0)
    # print(pstar_xayend.shape)

    pstar = jnp.concatenate(
        [pstar_xay0, \
        jnp.concatenate([pstar_x0ya, pstar_inner, pstar_xendya], axis=1), \
        pstar_xayend],axis=0)
    # print(pstar.shape)
...
def corrector(u, v, p,\
    ...
    pstar2_x0ya =jnp.expand_dims(pstar[1, 1:-1] \
            - c2_dtdx * (ustar[1, 1:-1] - ustar[0, 1:-1]) \
            - c2_dtdy * (vstar[1, 1:-1] - vstar[1, :-2]),axis=1)
    # print(pstar2_x0ya.shape)
    pstar2_xendya = jnp.expand_dims(pstar[-2, 1:-1] \
            - c2_dtdx * (ustar[-2, 1:-1] - ustar[-3, 1:-1]) \
            - c2_dtdy * (vstar[-2, 1:-1] - vstar[-2, :-2]),axis=1)
    # print(pstar2_xendya.shape)
    pstar2_xay0 = jnp.expand_dims(jnp.r_[0.0,
                pstar[1:-1, 1] \
            - c2_dtdx * (ustar[1:-1, 1] - ustar[:-2, 1]) \
            - c2_dtdy * (vstar[1:-1, 1] - vstar[1:-1, 0]),
                0.0],axis=0)
    # print(pstar2_xay0.shape)
    pstar2_xayend = jnp.expand_dims(jnp.r_[0.0,
                pstar[1:-1, -2] \
            - c2_dtdx * (ustar[1:-1, -2] - ustar[:-2, -2]) \
            - c2_dtdy * (vstar[1:-1, -2] - vstar[1:-1, -3]),
                0.0],axis=0)
    pstar2 = jnp.concatenate(
        [pstar2_xay0, \
        jnp.concatenate([pstar2_x0ya, pstar2_inner, pstar2_xendya], axis=1), \
        pstar2_xayend],axis=0)
