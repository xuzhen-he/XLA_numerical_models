                          ...
                          value_t u0)
{
    ...
    if (i < Nx && j < Ny)
    {
        if (i == 0) // i = 0 y = all
        {
            ...
            v_star[gid] = 0.0;
            gid = gid + Ny;
            p_star[gid - Ny] = p[gid] - c2_dtdx * (u[gid + Ny] - u[gid]) - c2_dtdy * (v[gid + 1] - v[gid]);
        }
        ...
        else
        {
            if (j == 0) // i = all except for two ends y = 0
            {
                ...
                v_star[gid] = 0.0;
                gid = gid + 1;
                p_star[gid - 1] = p[gid] - c2_dtdx * (u[gid + Ny] - u[gid]) - c2_dtdy * (v[gid + 1] - v[gid]);
            }
            ...
            {
                u_star[gid] = u[gid] - dtdx * (u[gid] * (u[gid + Ny] - u[gid]) + p[gid + Ny] - p[gid]) - dtdy * v[gid] * (u[gid + 1] - u[gid]) + nu_dtdxx * (u[gid + Ny] - 2.0 * u[gid] + u[gid - Ny]) + nu_dtdyy * (u[gid + 1] - 2.0 * u[gid] + u[gid - 1]);
                v_star[gid] = v[gid] - dtdx * u[gid] * (v[gid + Ny] - v[gid]) - dtdy * (v[gid] * (v[gid + 1] - v[gid]) + p[gid + 1] - p[gid]) + nu_dtdxx * (v[gid + Ny] - 2.0 * v[gid] + v[gid - Ny]) + nu_dtdyy * (v[gid + 1] - 2.0 * v[gid] + v[gid - 1]);
                p_star[gid] = p[gid] - c2_dtdx * (u[gid + Ny] - u[gid]) - c2_dtdy * (v[gid + 1] - v[gid]);
            }
        }
    }
}
                          ...
                          value_t u0)
{
    ...
    if (i < Nx && j < Ny)
    {
        if (i == 0) // i = 0 y = all
        {
            ...
            v[gid] = 0.0;
            gid = gid + Ny;
            value_t p_star2 = p_star[gid] - c2_dtdx * (u_star[gid] - u_star[gid - Ny]) - c2_dtdy * (v_star[gid] - v_star[gid - 1]);
            p[gid - Ny] = 0.5 * (p[gid] + p_star2);
        }
        else if (i == Nx - 1) // i = end y = all
        {
            u[gid] = 0.0;
        ...
        else
        {
            ...
            else
            {
                ...
                u_star2 = u_star[gid] - dtdx * (u_star[gid] * (u_star[gid] - u_star[gid - Ny]) + p_star[gid] - p_star[gid - Ny]) - dtdy * v_star[gid] * (u_star[gid] - u_star[gid - 1]) + nu_dtdxx * (u_star[gid + Ny] - 2.0 * u_star[gid] + u_star[gid - Ny]) + nu_dtdyy * (u_star[gid + 1] - 2.0 * u_star[gid] + u_star[gid - 1]);
                v_star2 = v_star[gid] - dtdx * u_star[gid] * (v_star[gid] - v_star[gid - Ny]) - dtdy * (v_star[gid] * (v_star[gid] - v_star[gid - 1]) + p_star[gid] - p_star[gid - 1]) + nu_dtdxx * (v_star[gid + Ny] - 2.0 * v_star[gid] + v_star[gid - Ny]) + nu_dtdyy * (v_star[gid + 1] - 2.0 * v_star[gid] + v_star[gid - 1]);
                p_star2 = p_star[gid] - c2_dtdx * (u_star[gid] - u_star[gid - Ny]) - c2_dtdy * (v_star[gid] - v_star[gid - 1]);
                u[gid] = 0.5 * (u[gid] + u_star2);
                v[gid] = 0.5 * (v[gid] + v_star2);
                p[gid] = 0.5 * (p[gid] + p_star2);
            }
        }
    }
}
